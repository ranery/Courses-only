...
net = torch.load('./models/FCNs-BCEWithLogits_batch6_epoch500_RMSprop_scheduler-step50-gamma0.5_lr0.0001_momentum0_w_decay1e-05')
...
root_dir   = "/home/zhoupan/disk1/cityscapes/"
val_file   = os.path.join(root_dir, "test.csv")
val_data = CityscapesDataset(csv_file=val_file, phase='val', flip_rate=1)
...
for iter, batch in enumerate(val_loader):
	...
	output = net(inputs)
	inputs = inputs.data.cpu().numpy()[0]
	output = output.data.cpu().numpy()
	N, cls, h, w = output.shape
	# print(output.shape)
	pred = output.transpose(0,2,3,1).reshape(-1, cls).argmax(axis=1).reshape(N, h, w)

	output = output[0].transpose(1,2,0).argmax(axis=2)
	h, w = output.shape
	lab = np.zeros([h, w, 3])
	for i in range(h):
		for j in range(w):
			lab[i, j] = index2color[output[i, j]]
